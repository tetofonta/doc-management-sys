...
export const RemoteComponent = ({component_id, error, load, use_short_error, ...oth}: RemoteComponentProps) => {
    ...
    if( !components[component_id] || !components[component_id].index )
    ...
    return <RemoteComponentLoader
        ...
        component_url={components[component_id].index}
        integrity={components[component_id].integrity}
        load={load || <Loading />}
        error={error || ((m: string) => <MessageBox small={use_short_error} title={"Loading error"} message={m} type={MessageDialogType.ERROR}/>)}
        {...oth}
    />
}