...
export class ListDocumentsFeature {
    constructor(
    ...
    public async listDocuments(q: SearchQuery, user: string, groups: string[]): Promise<object> {
            ...
            searches: [
                {
                    ...
                    per_page: q.take,
                    q: search_data.q,
                    filter_by: this.qs.assemble(search_data.filters, undefined, ['tag']),
                    query_by: 'title,embedding',
                    infix: 'always,off',
                    facet_by: 'category,owner,group',
                    include_fields: 'category,owner,group,title,$tag(tag, strategy: nest_array),id',
        ...
        return {
            ...
            result: raw.data.results[0].hits?.map((e) => ({
                ...e.document,
                tag: e.document?.tag?.map((t) => t?.tag).filter((t) => !!t),
            })),
            facets: raw.data.results[0].facet_counts.reduce((acc, facet) => {
                acc[facet.field_name] = facet.counts.map((e) => ({ name: e.value, count: e.count }));
                return acc;
            }, {}),
        };
    }
